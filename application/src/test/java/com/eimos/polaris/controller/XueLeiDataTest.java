...
class XueLeiDataTest {
    public static final String PRODUCTS = "产品用途场景\t消杀\n" +
            "产品用途场景\t美妆\n" +
            "产品用途场景\t日化 \n" +
            "产品用途场景\t同业\n" +
            "产品功能\t皮肤消杀\n" +
    ...
    public void test() {
        ...
        // 产品、物料主数据新增列
        this.createAttribute(XueLeiDataTest.PRODUCTS, MasterDataType.PRODUCT);
        this.createAttribute(XueLeiDataTest.MATERIALS, MasterDataType.MATERIAL);
    }

    private void createAttribute(final String products, final MasterDataType product) {
        final String[] rows = products.split("\n");
        final List<String> columns = Arrays.stream(rows).map(r -> r.split("\t"))
                .map(s -> s[0])
                .distinct()
                .collect(Collectors.toList());
        final Set<String> attributes = product.basicModel().getAttributes().stream().map(AttributeVo::getName).collect(Collectors.toSet());
        for (final String column : columns) {
            if (!attributes.contains(column)) {
                this.masterDataController.create(product.getName(), AttributeVo.fk2bd(column));
            }
        }
    }
    ...
    private void productMaterial() {
        final String[] rows = XueLeiDataTest.PRODUCTS.split("\n");
        final String[] rows1 = XueLeiDataTest.MATERIALS.split("\n");
