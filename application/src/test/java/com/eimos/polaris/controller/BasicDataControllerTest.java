...
class BasicDataControllerTest {
    ...
    void test() {
        // 1. 创建基础数据实体
        final String entityCode = "单元测试";
        this.controller.create(new EntityVo(entityCode, entityCode));

        // 2. 查询基础数据实体
        final List<EntityVo> entities = this.controller.entities(entityCode, 10, 1);
        Assertions.assertNotNull(entities);
        Assertions.assertEquals(entities.size(), 1);

        // 3. 创建数据，查询验证
        final String code = "01";
        this.controller.add(entityCode, new BasicDataVo(code, "水"));
        final List<BasicDataVo> list = this.controller.list(entityCode, code, 10, 1);
        Assertions.assertNotNull(list);
        Assertions.assertEquals(list.size(), 1);
        Assertions.assertEquals(list.get(0).getName(), "水");

        // 4. 修改数据，查询验证
        this.controller.modify(entityCode, new BasicDataVo(code, "电"));
        final List<BasicDataVo> list1 = this.controller.list(entityCode, code, 10, 1);
        Assertions.assertNotNull(list1);
        Assertions.assertEquals(list1.size(), 1);
        Assertions.assertEquals(list1.get(0).getName(), "电");

        // 5. 删除数据，查询验证
        this.controller.delete(entityCode, code);
        final List<BasicDataVo> list2 = this.controller.list(entityCode, code, 10, 1);
        Assertions.assertNotNull(list2);
        Assertions.assertEquals(list2.size(), 0);

        // 6. 删除基础数据实体，查询验证
        this.controller.drop(entityCode, false);
        final List<EntityVo> entities1 = this.controller.entities(entityCode, 10, 1);
    ...
    @Test
    public void xueLei() {
        final List<EntityVo> entityVos = this.controller.entities("", 100, 1);
        for (final EntityVo entityVo : entityVos) {
            this.controller.drop(entityVo.getName(), false);
        }
        ...
        for (final Map.Entry<String, List<Pair<String, String>>> entry : entities.entrySet()) {
            ...
            for (int i = 0; i < items.size(); i++) {
                ...
                this.controller.add(entityName, new BasicDataVo(String.format("%2d", i), v));
            }
        }
    }
}